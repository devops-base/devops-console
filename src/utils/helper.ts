 ...
 * 根据id获取值
 */
export function valueToId(value: number, arr: IConstant[]): string {
  for (let i = 0; i < arr?.length;i++) {
    if (arr[i].id === value) {
      return arr[i].label;
    }
...
// 时间参数
export const dateFormat = (dateStr: Date|string |undefined, formatter = 'yyyy-MM-dd hh:mm:ss') => {
  if (!dateStr) {
    return '';
  }
  ...
  formatter = formatter.replace(/(y+)/, (match, group) => {
    return (date.getFullYear() + '').substring(4 - group.length);
  });
  for (const [k, v] of Object.entries(ptr)) {
    formatter = formatter.replace(new RegExp('(' + k + ')'), (match, group) => {
      return group.length === 1 ? '' + v : ('00' + v).substring(('' + v).length);
    });
  }
  return formatter;
};
