<template>
  ...
  >
    ...
    <div v-if="menus.length > 0" class="h-full min-w-1024px">
...
import {getUserInfo, getUserMenus, updatePassword} from '@/servers/login';
...
onMounted(() => {
  ...
  // 如果用户id不存在则重新获取
  if (token && !userInfo.value?.userId) {
    getMenuData().then();
    getUser().then();
  }
});
...
const getMenuData = async () => {
  try {
    ...
    if (!data.length || !token) {
      return message.error({content: '用户暂无权限登陆', key: 'permission'});
    }
    setMenus(data);
  } finally {
    isLoading.value = false;
